...
int main() {
...
#ifdef _WIN32
    ImFontConfig baseCfg; baseCfg.SizePixels = 22.0f;
    io.Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\segoeui.ttf", 22.0f, &baseCfg, io.Fonts->GetGlyphRangesDefault());
    ImFontConfig symCfg; symCfg.SizePixels = 22.0f; symCfg.MergeMode = true; symCfg.GlyphMinAdvanceX = 22.0f;
    io.Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\seguisym.ttf", 22.0f, &symCfg, symRanges.Data);
#else
    const char* fonts[] = {
    ...
    for (int i = 0; fonts[i]; i++) {
        ...
        if (f) { fclose(f); io.Fonts->AddFontFromFileTTF(fonts[i], 22.0f, nullptr, symRanges.Data); loaded = true; break; }
    }
